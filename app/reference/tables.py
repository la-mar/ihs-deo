...
import logging
import warnings #? Ignore SAWarnings
from datetime import datetime

import pandas as pd
from sqlalchemy import Column, FetchedValue, MetaData, Table, create_engine
...
from app.settings import DATABASE_URI, LOGLEVEL
...
class dbagent(object):
    ...
    def __init__(self, dbname, tables: list = None):
        ...
        self.engine = create_engine(DATABASE_URI+dbname, logging_name = dbname)
...
class GenericTable(object):
    ...
    @classmethod
    def ids_updated_since(cls, dt: datetime = None):
        dt = dt or datetime.strptime('1970', '%Y')
        query = cls.session.query(*cls.get_pk_cols()).filter(cls.updated > dt)
        return pd.read_sql(query.statement, query.session.bind)
...
class Pb_Well_Test(GenericTable, Base):
    """ IHS.dbo.PB_WELL_TEST"""

    __table_args__ = {'autoload': True,
                      'autoload_with': ihs.engine,
                      'schema': 'dbo'}
    __tablename__ = 'PB_WELL_TEST'
    # id = Column('id', Integer, primary_key=True, autoincrement = True)
    api10 = Column('api10', String, FetchedValue())
    days_since_test = Column('days_since_test', String, FetchedValue())
    session = ihs.Session()
...
if __name__ == "__main__":
    ...
    x = Pb_Well_Test.ids_updated_since('2019-03-01')
    x
    # x = FracSchedule.get_last_update()


    # IPTest.records_updated_since(dt)
