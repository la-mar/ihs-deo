...
{
    /// <summary>
    /// Exports data using query name
    /// </summary>
    public class QueryNameExportStrategy : IExportStrategy
    {
        /// <summary>
        /// Exports data
        /// </summary>
            ...
            using (var queryBuilder = new QueryBuilder(session))
            {
                if (!string.IsNullOrWhiteSpace(options.LastUpdateDate))
                {
                    ...
                    if (DateTime.TryParseExact(options.LastUpdateDate,
                        ...
                        out oDate))
                    {
                        var queryString = queryBuilder.GetQueryDefinition(queryName);
                        ...
                        if (lastUpdateCriteria == null)
                        {
                           // var newCriteria = CriteriaService.CreateLastUpdateCriteria(valueId.ToString(), options.Datatype, attribute, CriteriaService.FilterLogic.GreaterThanOrEquals, oDate , options.Domain);
                            var newCriteria = CriteriaService.CreateLastUpdateCriteria(valueId.ToString(), options.Datatype, attribute, oDate,oDate, options.Domain);
                        ...
                        else
                        {
                           ...
                           var newCriteria = CriteriaService.CreateLastUpdateCriteria((valueId - 1).ToString(), options.Datatype, attribute,  oDate, oDate,options.Domain);
